...
contract C11A172 {
    ...
    C11A171 public tokenContract;

    constructor(address _tokenContract) {
        tokenContract = C11A171(_tokenContract);
    }
...
contract C11A173 {
    ...
    C11A171 private token;
    C11A172 private depositProvider;
    address private owner;
    uint private called;
    uint private limit = 2;

    constructor(address _depositContract, address _tokenContract) {
        token = C11A171(_tokenContract);
        depositProvider = C11A172(_depositContract);
        owner = msg.sender;
        ...
        external 
    {
        require(
            ...
            "Hook is called by C11A171 only"
        );
        called += 1;
        if(called < limit) {
